...
public abstract class AttributeOld {
	...
	public AttributeOld(String name, String desc, Class<? extends CoreAbility> ability) {
		this.name = name;
		this.desc = desc;
		...
		for (int i = 0; i < getters.length; i++) {
			...
			for (CoreAbility ability : CoreAbility.getAbilities()) {
				...
				try {
					...
					registerAttribute(name, desc, ability.getClass(), getters[i], setters[i]);
					
				} catch (NoSuchMethodException e) {}
			}
		}
		
		registerAttribute("RaiseEarthWallWidth", "Sets the Width of EarthRaise", RaiseEarthWall.class, "getWidth", "setWidth");
		registerAttribute("RaiseEarthWallHeight", "Sets the Height of EarthRaise", RaiseEarthWall.class, "getHeight", "setHeight");
	 ...
	 */
	/*public static void registerAttribute(String name, String desc, Class<? extends CoreAbility> ability, String getter, String setter) {
		if (attributeList.containsKey(name.toLowerCase())) {
			if (attributeList.get(name.toLowerCase()) instanceof DefaultAttribute) {
				((DefaultAttribute)attributeList.get(name.toLowerCase())).addAbility(ability);
			}
		} else {
			Attribute attribute = new DefaultAttribute(name, desc, ability, getter, setter);
			attributeList.put(name.toLowerCase(), attribute);
		}
