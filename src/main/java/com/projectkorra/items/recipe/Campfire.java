...
public class Campfire extends RecipeParser<CampfireRecipe> {
    ...
    @Override
    public CampfireRecipe parseRecipe(ConfigurationSection section, PKItem item) {
        ...
        float experience = (float)section.getDouble("Experience", 0.01D);
        int burntime = section.getInt("CookTime", 600);
        RecipeChoice recipeIngredient = getIngredient(ingredient, item);
        if (recipeIngredient != null) { //If an error didn't occur
            NamespacedKey key = new NamespacedKey(ProjectKorraItems.plugin, "campfire/" + item.getInternalName());
            ItemStack stack = item.buildStack(null);
            stack.setAmount(amount);
            CampfireRecipe recipe = new CampfireRecipe(key, stack, recipeIngredient, experience, burntime);
    ...
    @Override
    public boolean registerRecipe(CampfireRecipe recipe) {
