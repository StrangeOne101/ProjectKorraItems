...
public class Smoker extends RecipeParser<SmokingRecipe> {
    ...
    @Override
    public SmokingRecipe parseRecipe(ConfigurationSection section, PKItem item) {
        ...
        float experience = (float)section.getDouble("Experience", 0.02D);
        ...
        if (recipeIngredient != null) { //If an error didn't occur
            NamespacedKey key = new NamespacedKey(ProjectKorraItems.plugin, "smoker/" +item.getInternalName());
            ItemStack stack = item.buildStack(null);
            stack.setAmount(amount);
            SmokingRecipe recipe = new SmokingRecipe(key, stack, recipeIngredient, experience, burntime);
    ...
    @Override
    public boolean registerRecipe(SmokingRecipe recipe) {
