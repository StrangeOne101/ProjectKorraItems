...
public class Stonecutter extends RecipeParser<StonecuttingRecipe> {
    ...
    @Override
    public StonecuttingRecipe parseRecipe(ConfigurationSection section, PKItem item) {
        ...
        if (recipeIngredient != null) { //If an error didn't occur
            NamespacedKey key = new NamespacedKey(ProjectKorraItems.plugin, "stone/" + item.getInternalName());
            ItemStack stack = item.buildStack(null);
            stack.setAmount(amount);
            StonecuttingRecipe recipe = new StonecuttingRecipe(key, stack, recipeIngredient);
    ...
    @Override
    public boolean registerRecipe(StonecuttingRecipe recipe) {
