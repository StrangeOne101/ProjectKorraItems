...
public class MenuBase implements InventoryHolder {
	 ...
	 *            The Slot clicked
	 */
	protected boolean onMenuClick(Player player, int index, ClickType click, ItemStack cursor) {
	...
	public void openMenu(Player player) {
		if (getInventory().getViewers().contains(player)) {
			// throw new IllegalArgumentException(player.getName() + " is
			// already viewing " + getInventory().getTitle());
			return;
		}
	...
	/**
	 * Closes this menu and opens another in the next tick. Avoids Bukkit
	 * glitchiness. caused by closing and opening inventories in the same tick.
	 */
	public void switchMenu(final Player player, MenuBase toMenu) {
		...
		}.runTask(ProjectKorraItems.plugin);
	}
	 ...
	 */
	/*
	 * public void updateMenu() { for (HumanEntity entity :
	 * getInventory().getViewers()) { if (entity instanceof Player) { Player
	 * player = (Player) entity; player.updateInventory(); } } }
	 */
}
