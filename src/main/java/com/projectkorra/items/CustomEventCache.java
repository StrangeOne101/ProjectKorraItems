...
import com.projectkorra.items.api.itemevent.Target;
...
public class CustomEventCache {
    ...
    //Registry
    static Map<CustomItem, Map<Class<? extends Event>, Set<BiConsumer<Event, EventContext>>>> ITEMEVENT_EXECUTORS = new HashMap<>();

    static Map<Class<? extends Event>, Map<BiConsumer<Event, EventContext>, MutableTriple<Set<CustomItem>, Target, ActiveConditions>>> METHODS_BY_EVENT = new HashMap<>();
    ...
    public static void fullCache(Player player) {
        ...
        CustomEventCache.DIRTY_INVENTORY.remove(player);
    ...
    static void triggerItemEvents(Event event) {
        ...
        for (BiConsumer<Event, EventContext> m : METHODS_BY_EVENT.get(event.getClass()).keySet()) {
            ...
            for (CustomItem item : triple.getLeft()) {
                if (triple.getRight() == ActiveConditions.NONE) { //We don't execute it based on whether the item is active
                    ...
                    } else if (event instanceof AbilityStartEvent) {
                        player = ((AbilityStartEvent) event).getAbility().getPlayer();
                    } else if (event instanceof AbilityEndEvent) {
                        player = ((AbilityEndEvent) event).getAbility().getPlayer();
                    } else if (event instanceof AbilityProgressEvent) {
                        player = ((AbilityProgressEvent) event).getAbility().getPlayer();
                    } else if (event instanceof AbilityDamageEntityEvent && ((AbilityDamageEntityEvent) event).getEntity() instanceof Player) {
                        player = (Player) ((AbilityDamageEntityEvent) event).getEntity();
                    } else {
                    ...
                    else if (event instanceof AbilityStartEvent) world = ((AbilityStartEvent)event).getAbility().getPlayer().getWorld();
                    else if (event instanceof AbilityProgressEvent) world = ((AbilityProgressEvent)event).getAbility().getPlayer().getWorld();
                    else if (event instanceof AbilityEndEvent) world = ((AbilityEndEvent)event).getAbility().getPlayer().getWorld();
                    else if (event instanceof AbilityDamageEntityEvent) world = ((AbilityDamageEntityEvent)event).getAbility().getPlayer().getWorld();
    ...
    public static <T extends Event> void registerItem(CustomItem item, ActiveConditions conditions, Target target, Class<T> eventClass, BiConsumer<T, EventContext> method) {
        ...
        if (!ITEMEVENT_EXECUTORS.get(item).containsKey(eventClass)) {
            ITEMEVENT_EXECUTORS.get(item).put(eventClass, new HashSet<>());
        }

        if (!METHODS_BY_EVENT.containsKey(eventClass)) {
            METHODS_BY_EVENT.put(eventClass, new HashMap<>());
        }

        Set<CustomItem> items = new HashSet<>();
        if (METHODS_BY_EVENT.get(eventClass).containsKey(method)) {          //If the method is already registered for
            items = METHODS_BY_EVENT.get(eventClass).get(method).getLeft();  //another item (multiple items, single class)
        }
        items.add(item);

        ITEMEVENT_EXECUTORS.get(item).get(eventClass).add((BiConsumer<Event, EventContext>) method);
        METHODS_BY_EVENT.get(eventClass).put((BiConsumer<Event, EventContext>) method, new MutableTriple<>(items, target, conditions));

        //Make sure bukkit will trigger our event methods
        if (!REGISTERED_EVENT_HANDLERS.contains(eventClass)) {
            registerItemEventListener(eventClass);
        }
